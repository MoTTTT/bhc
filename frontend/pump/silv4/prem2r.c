...
file_open()
	...
	sprintf( karakter, "%sp2%02d_%02d.doc",wcr_env, my_time->tm_mon+1, my_time->tm_mday);
...
void check_data()
	...
	if( old_minute != MIN)
		...
		sprintf ( tmp_buf, " %.1f", FLOWT);
		OuttextXY(5, 297, tmp_buf, strlen(tmp_buf), YELLOW);
		strcat ( rx_buffer, tmp_buf);
		memset( tmp_buf, 0, sizeof( tmp_buf));
		sprintf ( tmp_buf, " %.2f", PRESSURET1);
		strcat ( rx_buffer, tmp_buf);
		sprintf ( tmp_buf, " %.2f", PRESSURET2);
		strcat ( rx_buffer, tmp_buf);
		sprintf ( tmp_buf, " %.2f", PRESSURET3);
		strcat ( rx_buffer, tmp_buf);
...
prem4()
	...
	outtextxy( (MaxX-250)/2  , ypos      , "PUMP STATION 4");
	setcolor(EGA_WHITE);
	...
	outtextxy( xpos+415, ypos+310 , "Tansdcer");

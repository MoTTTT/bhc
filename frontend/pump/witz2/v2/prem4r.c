...
long GlobalDel;
float  devider_25 =(float) MAKS_BITS/2000, devider_10 =(float) 1646/10;
float  FLOW=0, FLOWT, PRESSURE1=0, PRESSURET1=0, PRESSURE2=0, PRESSURET2=0, PRESSURE3=0, PRESSURET3=0;
float  PRESSURET4=0, PRESSURE4=0;
long peela_c=0, pressure_c1=0,  pressure_c2=0, pressure_c3=0, pressure_c4=0;
...
/** HEAD FLOW meter **/
	barv_length =(float) MAKS_FLOW/v_maks[7];
	value = (float) Waardes[7];
	barv_length *= value;
	if ( Waardes[7] < 0)
		color = 1;
	else
...
/** Pressure transducer 4**/
	barv_length =(float) MAKS_PRESSURE/v_maks[3];
	...
	if ( Waardes[3] < 0)
	 ...
	 color=0;
	sprintf ( date_buf, "%4.0f Kpa", barv_length);
	Outtextxy (400, 332  , date_buf, 8, color);
...
file_open()
	...
	sprintf( karakter, "%s\\pr3%02d_%02d.doc",wcr_env,\
...
void check_data()
	...
	if( old_minute != MIN)
		...
		PRESSURET4 = PRESSURE4/pressure_c4;
		sprintf ( tmp_buf, "%12.1f", FLOWT);
		OuttextXY(5, 297, tmp_buf, strlen(tmp_buf), YELLOW);
		memset( tmp_buf, 0, sizeof( tmp_buf));
		sprintf ( tmp_buf, " %.1f %.2f %.2f %.2f %.2f\n",\
		    FLOWT, PRESSURET1, PRESSURET2, PRESSURET3, PRESSURET4);
		strcat ( rx_buffer, tmp_buf);
		...
		peela_c = pressure_c1 = pressure_c2=pressure_c3 =pressure_c4=0;
		old_minute = MIN;
		}
	if( old_day != DAY)
		{
...
prem4()
	...
	line( xpos+15, ypos+55, xpos+25, ypos+55);
	rectangle ( xpos+25, ypos+53, xpos+35, ypos+57);
	...
	outtextxy( xpos+405, ypos+300 , "Pressure");
	outtextxy( xpos+405, ypos+310 , "Tansducer");

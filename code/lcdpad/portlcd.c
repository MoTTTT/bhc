...
/* 23:10pm 07-22-1997 	Initialisation problems fixed			*/
/* 23:12pm 07-22-1997 	Problems with i2c bus?				*/
/*									*/
...
char	set_plcd	( char in );	/* Control LCD			*/
...
{
char	i= 1;
	set_plcd( LCD_I0 );		/* See #define's for details	*/
	set_plcd( LCD_I1 );		/* See #define's for details	*/
	set_plcd( LCD_I2 );		/* See #define's for details	*/
	set_plcd( LCD_I3 );		/* See #define's for details	*/
...
char	set_plcd	( char in )	/* Control LCD			*/
{
	...
	return ( pputchar( in ));	/* Send control settings	*/
}
